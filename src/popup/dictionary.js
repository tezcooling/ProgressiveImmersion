...
browser.storage.local.get('dictionary').then(value => {
    ...
    document.getElementById('import-file').addEventListener('change', async e => {
        ...
        try {
            ...
            if (importedData[originIso] && importedData[originIso][targetIso]) {
                value.dictionary[originIso][targetIso] = { ...importedData[originIso][targetIso] };
                browser.storage.local.set({ dictionary: value.dictionary });

                // Clear current table and redraw with imported data
                while (dictionary.rows.length > 2) {
                    dictionary.deleteRow(1);
                }
                ...
                alert('Imported file does not contain the expected dictionary format.');
            }
        } catch (error) {
    ...
    document.getElementById('export-button').addEventListener('click', e => {
        if (Object.keys(value.dictionary[originIso][targetIso]).length === 0) {
            alert('Nothing to export. Your dictionary is empty.');
            return;
        }
